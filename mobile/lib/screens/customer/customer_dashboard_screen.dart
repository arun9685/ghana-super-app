...
class _CustomerDashboardScreenState extends State<CustomerDashboardScreen> {
  ...
  Future<void> _loadActiveRide() async {
    try {
      final data =
          await ApiClient.instance.get('/rides', query: {'status': 'active'});
  ...
  static const _tiles = [
    {
      'key': 'move',
      'label': 'Move',
      'desc': 'Book a ride across Accra',
      'live': true
    },
    {
      'key': 'eat',
      'label': 'Eat',
      'desc': 'Order from kitchens near you',
      'live': true
    },
    {
      'key': 'utilities',
      'label': 'Utilities',
      'desc': 'Airtime, power and water',
      'live': true
    },
    {
      'key': 'fix',
      'label': 'Fix',
      'desc': 'Verified home service providers',
      'live': true
    },
    {
      'key': 'liquidity',
      'label': 'Liquidity',
      'desc': 'Wallet & micro-loans',
      'live': true
    },
    {
      'key': 'fleet',
      'label': 'Fleet',
      'desc': 'Manage company vehicles',
      'live': true
    },
    {
      'key': 'property',
      'label': 'Property',
      'desc': 'Rent or buy across Ghana',
      'live': true
    },
    {
      'key': 'travel',
      'label': 'Travel',
      'desc': 'Fly across the continent',
      'live': true
    },
  ];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Row(mainAxisSize: MainAxisSize.min, children: [
          SankofaMark(size: 28, light: true),
          SizedBox(width: 10),
          Text('Sankofa')
        ]),
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            Text('Hello, ${me?.name?.split(' ').first ?? 'there'}',
              ...
              Card(
                ...
                child: ListTile(
                  onTap: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (_) =>
                            RideTrackingScreen(rideId: _activeRide!.id)),
                  ),
                  leading: const Icon(Icons.circle,
                      color: Color(0xFF4ADE80), size: 10),
                  title: Text(
                      '${_activeRide!.pickupAddress} → ${_activeRide!.dropoffAddress}',
                      style: const TextStyle(
                          color: Colors.white, fontWeight: FontWeight.bold)),
                  subtitle: Text('Trip in progress · ${_activeRide!.status}',
                ...
                return _TileCard(
                  ...
                  onTap: live
                      ? () => Navigator.push(context,
                          MaterialPageRoute(builder: (_) => _screenFor(key)))
                      : null,
                );
              }).toList(),
            ),
