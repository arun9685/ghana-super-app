...
class TripMapWidget extends StatelessWidget {
  ...
  const TripMapWidget(
      {super.key, required this.pickup, required this.dropoff, this.driver});
  ...
  Widget build(BuildContext context) {
    return AspectRatio(
      ...
      child: CustomPaint(
        painter:
            _TripMapPainter(pickup: pickup, dropoff: dropoff, driver: driver),
        child: Container(),
      ),
    );
  }
...
class _TripMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Offset toXY(double lat, double lng) {
      final x =
          ((lng - minLng) / ((maxLng - minLng) == 0 ? 1 : (maxLng - minLng))) *
                  (size.width - 80) +
              40;
      final y = size.height -
          (((lat - minLat) / ((maxLat - minLat) == 0 ? 1 : (maxLat - minLat))) *
                  (size.height - 80) +
              40);
      return Offset(x, y);
    }
    ...
    canvas.drawCircle(
        ...
        Paint()
          ...
          ..strokeWidth = 3);
    _drawLabel(
        canvas, 'Pickup', p + const Offset(14, -4), const Color(0xFF0B0F0D));
    ...
    canvas.drawCircle(
        ...
        Paint()
          ...
          ..strokeWidth = 3);
    _drawLabel(
        canvas, 'Drop-off', d + const Offset(14, -4), const Color(0xFF0B0F0D));
    ...
    if (driver != null) {
      ...
      canvas.drawCircle(
          ...
          Paint()
            ...
            ..strokeWidth = 3);
      _drawLabel(
          canvas, 'Driver', dr + const Offset(16, -4), const Color(0xFFB07D08));
    }
  }
  ...
  bool shouldRepaint(covariant _TripMapPainter oldDelegate) =>
      oldDelegate.pickup.lat != pickup.lat ||
      oldDelegate.driver?.lat != driver?.lat;
}
